...
namespace DAL
{
    class Activity
    {
        ...
        public Model.Activity GetRecommend()
        {
            using (IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "select * from activity where activityVerify='审核通过'and recomment='是' and endTome>=now() orderby recommendTime desc limit 1";
        ...
        public IEnumerable<Model.ActivityName> GetPage(Model.Page page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
